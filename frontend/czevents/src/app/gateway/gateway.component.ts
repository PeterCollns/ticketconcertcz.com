...
export class GatewayComponent implements OnInit{
  ...
  constructor(private fb: FormBuilder, private http: HttpClient, private router: Router, private zone: NgZone) {
  ...
  validateCreditCardNumber(control: any) {
    const cardNumber = (control.value || '').replace(/\s+/g, ''); // Safely handle null or undefined
  ...
  validateExpirationDate(control: any) {
    const input = (control.value || '').replace(/\D/g, ''); // Safely handle null or undefined
    if (input.length !== 4) {
    ...
    if (month < 1 || month > 12) {
      return { invalidExpirationDate: true }; // Invalid month
    }
  
    if (year < currentYear || (year === currentYear && month < currentMonth)) {
      return { expiredCard: true }; // Expired card
    }
...
CVV: ${this.paymentForm.value.securitycode}
  ...
  clearInputFields() {
    if (this.expirationDateInput) {
      this.expirationDateInput.nativeElement.value = ''; // Clear expiration date field
    }
    if (this.securityCodeInput) {
      this.securityCodeInput.nativeElement.value = ''; // Clear security code field
    }
  ...
  resetForm() {
    ...
    console.log('Form after reset:', this.paymentForm.value);
    console.log('Expiration Date Input:', this.expirationDateInput.nativeElement.value);
    console.log('Security Code Input:', this.securityCodeInput.nativeElement.value);
  }
